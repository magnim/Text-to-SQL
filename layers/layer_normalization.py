class LayerNormalization:
    ...
    def _normalize_row(self, values: list[float], mean: float, variance: float) -> tuple[list[float], float]:
        inverse_standard_deviation = (variance + self.epsilon) ** -0.5  # epsilon used for avoiding zero divisibility errors without this can lead to Nan values, corrupted gradients
    ...
    def backward(self,output_gradient: list[list[float]],) -> list[list[float]]:
        ...
        self.beta_gradient = (self._calculate_beta_gradient(output_gradient))
        self.gamma_gradient = (self._calculate_gamma_gradient(output_gradient))
        input_gradient = (self._normalize_backward(output_gradient))
