...
PROGRESS=ROOT/"training/lexical_structure_progress.json"
...
def main():
    ...
    for name,p in model.named_parameters():
        if name.startswith(("lexical_question_pool_score","lexical_clause_head","lexical_continuation_head")):
    ...
    for epoch in range(1,args.epochs+1):
        ...
        for start in range(0,len(order),64):
            ...
            for index in batch:
                item=items[index]
                qs,qe=item["schema_spans"]["question"]
                qids=torch.tensor(
                    item["input_ids"][qs:qe],dtype=torch.long
                )
                states=model.token_embedding(qids)
                weights=torch.softmax(
                    model.lexical_question_pool_score(states).squeeze(-1),
                    dim=0,
                )
                summaries.append(
                    (states*weights.unsqueeze(-1)).sum(dim=0)
                )
            ...
            cl=model.lexical_clause_head(summaries)
            col=model.lexical_continuation_head(summaries)
            c_loss=F.binary_cross_entropy_with_logits(
                cl,cts,pos_weight=pos_weight
            )
            cont_loss=F.cross_entropy(
                col,conts,weight=cont_weights
            )
